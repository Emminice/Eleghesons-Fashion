            ...
            <div class="table-scroll">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><strong style="white-space:nowrap">{{ $cat->name }}</strong></td>
                            <td style="font-size:12px;color:var(--gray);white-space:nowrap">{{ $cat->slug }}</td>
                            <td><span class="chip">{{ $cat->products_count }}</span></td>
                            <td>
                                <span class="status-badge {{ $cat->is_active ? 'delivered' : 'cancelled' }}">
                            ...
                            <td>
                                <form method="POST" action="{{ route('admin.categories.destroy', $cat) }}"
                                    ...
                                    <button type="submit" class="btn-danger"
                                            ...
                                            title="{{ $cat->products_count > 0 ? 'Has products — cannot delete' : 'Delete category' }}">
